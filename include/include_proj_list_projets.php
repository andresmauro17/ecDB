...
class Proj {
	public function ProjList() {
		...
		$owner_sql = "";
		$public_only = " WHERE project_public = 1 ";
		$owner = null;
		...
		if(isset($_GET['by'])) {
			...
			if($by == 'name')
			{
				$GetDataComponentsAll = "SELECT projects.*, members.login login, members.currency currency FROM projects, members WHERE  project_owner = member_id ".$public_only." ORDER by project_name ".$order."";
			}
			else
			{
				$GetDataComponentsAll = "SELECT projects.*, members.login login, members.currency currency FROM projects, members WHERE project_owner = member_id ".$public_only." ORDER by project_name ASC";
			}
		}
		else
		{
		...
		while($showDetails = mysqli_fetch_array($sql_exec)) {
			echo "<tr>";
				...
				echo '<td>';
					...
					$GetDataPrice = "SELECT sum(total) as total, m.currency FROM (SELECT cast(price as decimal(14, 2)) * projects_data_quantity AS total FROM projects_data JOIN `data` WHERE data.id = projects_data_component_id AND projects_data_project_id = ".$showDetails['project_id'].") AS project_total, projects p, members m where p.project_id = ".$showDetails['project_id']." group by m.currency";
